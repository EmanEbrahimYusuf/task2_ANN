...
#####################################
tk.Label(root, text="Enter Threashold", font="Bahnschrift 20", bg="#100E17", fg="green").place(x=600, y=210)
e3=Entry(root)
threshod = float('0'+e3.get())
e3.place(x=605,y=250)
#####################################
tk.Label(root, text="Add Bais", font="Bahnschrift 20", bg="#100E17", fg="green").place(x=600, y=320)
check_1 = IntVar()

addbais = Checkbutton(root, \
                 onvalue = 1, offvalue = 0 \
                    ,variable= check_1
                 ,bg="#100E30").place(x=600,y=370)

# ##########################################
b=Button(root,text="Apply",width=15,bg="#100E17",fg="#f44336",command=lambda :task(cols,specise[0],specise[1],float('0'+e.get()),int('0'+e2.get()),check_1.get(),float('0'+e3.get())))
